...
void jwObj_string_max( char *key, char *value, int len);
...
//
void jwOpen( struct jWriteControl *jwc, char *buffer, unsigned int buflen, enum jwNodeType rootType );
...
void jwObj_string_max(  struct jWriteControl *jwc,  char *key, char *value, int len);
