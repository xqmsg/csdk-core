...
/*! Specifies an XQ backend product. Generally used to denote which server URL a
 * call should be directed to.
 */
enum product_server {
...
/// Internal XQ method that is responsible for making remote calls to XQ
/// servers.
...
/// @param withToken Should an access token be sent ( if one is available ) ?
/// This will be true in most cases.
/// @param multipart Any additional files to be passed into the request. If this
/// object is defined, the request type will automatically be set to multipart (
/// curl_mime
struct xq_response xq_call(struct xq_config *config, enum product_server server,
                           enum call_method method, const char *serviceName,
                           const char *payload, _Bool withToken,
...
///
/// Any and all response objects generate from xq_call should always be
/// destroyed using this method, once all their data has been processed.
