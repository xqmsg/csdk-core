...
void jwPutstrMax( JWC_DECL char *str, int len );
...
// put "quoted" string to object
//
void jwObj_string_max( JWC_DECL char *key, char *value, int len )
{
    if(_jwObj( JWC_PARAM key ) == JWRITE_OK)
        jwPutstrMax( JWC_PARAM value, len );
}
 ...
 * \param[out] str the output buffer.  Should be 16 chars or more.
 */
void modp_itoa10(int32_t value, char* str)
{
    char* wstr=str;
 ...
 *
 * \param[in] value The value to process.
 * \param[out] str  The allocated output buffer.  Should be 32 chars or more.
 * \param[in] prec  Number of digits to the right of the decimal point.
 *    Can only be 0-9.
 */
void modp_dtoa2(double value, char* str, int prec)
{
